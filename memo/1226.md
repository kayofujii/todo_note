...
1. あるタスク 1 のチェックボックスをクリックすると、 タスクを未完了から完了の状態に変更したことを html から Vue に送れるようにする。
2. Vue 側でタスクの状態を変更したことを django の views.py に伝える updateTask メソッドを定義する。
3. updateTask メソッドから送られてきた内容を元に、task の内容を未完了から完了に変更し、db に保存する。
4. 保存できたら、その結果(レスポンス)を Vue に返す。
5. html でタスクにチェックボックスがつき、斜線をつけるようにする
  ...
  <div class="mt-5">
    <div class="card mb-2" v-for="task, index in tasks">
      <div class="card-body">
        <div class="form-check">
          {# 追加 #}
          <input class="form-check-input" type="checkbox" v-model="task.completed" v-on:change="updateTask(task)">
...
まず、タスクのチェックボックスである部分に、`v-model="task.completed" v-on:change="updateTask(task)"`を追加します。

v-model="task.completed" で、チェックをしたときに、連動して、vue 上で管理している task のデータ内容を変更できるようにしています。
今回の場合は、task が completed(完了)しているかどうかを vue 上で管理していて、task が完了したら、css で斜線を引くという処理をしたいので、定義しています。

v-on:change="updateTask(task)"を書くことで、チェックボックスをクリックしたときに、これから main.js 書く、task を更新する処理を実行できるようにします。

`<label class="form-check-label" :class="{'completed': task.completed}">`では、vue で管理しているタスクの値が変わったときに class を追加したり削除したりできるようにしました。
参考:
https://jp.vuejs.org/v2/guide/class-and-style.html

今回の場合、task.completed が true(タスクが完了)になったときに、class として、completed を追加します。

completed という class を、文字列に斜線を加えるという class にしたいので、style.css にそのように記述します。
...
2. Vue 側でタスクの状態を変更したことを django の views.py に伝える updateTask メソッドを定義します。

main.js に updateTask メソッドを記述してください。
...
基本的には createTask メソッドと同じなのですが、task という引数を渡していて、method が put になっているという違いがあります。

getTasks,createTask,updateTask も似ている部分が多いので、共通した部分を新たなメソッドとして作り、リファクタリングをしようと思います。

まず、下記のように sendRequest メソッドを定義します。
...
csrftoken と、header を定義します。

下記 url にも書いてあるとおり、new Headers();を使用することで、新しいヘッダーを定義することができます。
...
また、get メソッド以外はヘッダーに csrftoken をふくめる必要があるので、`myHeaders.set('X-CSRFToken', csrftoken)`と記述し、新しいヘッダを追加します。

fetch 以下は各処理で共通部分はそのままかき、違う部分は、if 文や引数にとっている、url, method, data を使用して書いていきます。

getTasks メソッドを下記のように書き換える事ができます。
だいぶスッキリしました!!
...
createTask,updateTask も下記のように書き換えたら完成です。
...
最後に、django の views.py で、vue から得たタスクを完了にする処理を書いていきます。
views.py に下記の put メソッドを書いてください。
...
まず、post メソッドと同じように、vue からの json データを辞書型に変更します。辞書型２変更した task のデータに、task の id が含まれているので、それをもとに、`Task.objects.get(id=response.get("id"))`で、vue から送られてきた task の id と一致する django 側の db に保存された task を取得します。
取得できたら、タスクの現在の状態が完了していたら、未完了に、 タスクが未完了なら、完了にデータを書き換えて、db に保存します。

タスクのチェックボックスをクリックしてみて、斜線がつくことを確認できたら完了です!

いよいよ、最後の機能としてタスクを削除する機能を作っていきたいと思います。
下記の流れで実装を進めていきます。

1. あるタスク 1 のバツボタンをクリックすると、タスクを削除処理をしたこと html から Vue に 送れるようにする。
2. Vue 側でタスクを削除したことを django の views.py に伝える deleteTask メソッドを定義する。
3. deleteTask メソッドから送られてきた内容を元に、db に保存されていた、task を削除する。
4. 削除できたら、その結果(レスポンス)を Vue に返す。
5. html で削除したタスクは表示されないようにする。

まず、1 から始めます。html の削除ボタンの部分を変更してください。
...
追加したのは、`@click="deleteTask(task)"`で、クリックしたら、これから Vue で定義する deleteTask の処理を行なうようにしました。

2.続いて、main.js に deleteTask の処理内容を書いていきます。
...
vue のメソッドである、deleteTask は updateTask とほとんど処理が同じです。
違うのは、views.py で、データの値を変えるか、削除するかだけです。

では、最後に views.py を編集して完了です。
...
vue のデータを元に django から該当のタスクを取得するまでの処理は同じです。
取得したタスクを`task.delete()`で削除します。

コードをすべて書き終わったので、実際バツボタンを押してみてください。
削除できたら完成です!

最後に任意で html に bootstrap でデザインした、web ページのヘッダーををつけます。
アプリっぽくなるので、つけたい方はつけてみてください。
      ...
      <div class="row">
        ...
        {# 追加 #}
          <div class="jumbotron jumbotron-fluid">
            <div class="container">
                <h1 class="display-4">Simple Todo</h1>
                <p class="lead">シンプルさを極めたTodoリスト</p>
            </div>
          </div>
          {% block content %}
          {% endblock %}
...
読んでくださってありがとうございました。バグ等ありましたら、コメントいただけると幸いです。
これからも django や vue についての教材を作っていきたいと思いますので、いいねをいただけるととても嬉しいです!
また、YouTube note もやっていますので、チェックしていただけると嬉しいです。
